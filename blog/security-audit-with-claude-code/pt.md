...
Eu não estava tentando substituir um teste de penetração profissional. Queria uma primeira passagem completa, o tipo de revisão que pega os erros óbvios e os não tão óbvios antes que um especialista humano começasse.

O que torna o Claude Code interessante para isso é que ele consegue ler todo o seu codebase, entender a arquitetura, seguir fluxos de dados entre camadas e pensar como um atacante. Ele vai além do pattern matching por `eval()` ou secrets hardcoded e entra na lógica de negócio: perguntas como "o que acontece se um usuário enviar esse ID que pertence a outra pessoa?" ou "e se esse campo inteiro não tiver limite máximo?"
...
A auditoria vale o que vale a instrução. Eu não disse apenas "encontre bugs de segurança." Escrevi um prompt detalhado que enquadra o Claude Code em um papel específico, fornece um framework de análise estruturado e diz exatamente qual saída eu espero.

Aqui está o prompt completo (longo de propósito):
...
Também forneci as credenciais de teste e o endpoint da API live, para que ele pudesse fazer requisições HTTP reais e confirmar os achados contra a aplicação em execução, em vez de só adivinhar pelo código-fonte.

O enquadramento carregou a maior parte do peso: **"You are not a developer checking boxes — you are a security specialist who thinks like an attacker."** Essa pergunta produz uma análise completamente diferente de "esse código segue boas práticas?", e bem mais útil.

A estrutura se pagou de forma direta. Cada instrução específica do briefing voltou como uma seção específica e acionável no relatório; o pouco que deixei vago voltou vago.
...
O Claude Code traçou o fluxo de dados pelo endpoint de check de rotina e descobriu que, embora a requisição incluísse um parâmetro `userId`, o service **nunca o usava** para verificar propriedade. Qualquer usuário autenticado poderia marcar hábitos na rotina de qualquer outro usuário, e o XP seria concedido ao atacante em vez de ao dono da rotina.

Era um IDOR clássico (Insecure Direct Object Reference), mas sutil, porque o `userId` estava na assinatura do método: parecia que estava sendo usado. Você tinha que ler a implementação cuidadosamente para notar que era silenciosamente ignorado.
...
**A maioria das vulnerabilidades são bugs de lógica.** Ninguém ia fazer SQL injection no Beyou; o JPA parametriza tudo. Os riscos reais moravam na lógica de negócio em torno de XP, propriedade e limites de entrada.
...
**Auditoria assistida por IA acelera, e a discussão é metade do valor.** O Claude Code encontrou coisas que eu não teria pensado em verificar, e encontrou rápido. A fase em que expliquei minha arquitetura e refinamos os achados juntos importou na mesma medida. A ferramenta rende mais quando você discute com ela como com um colega, em vez de consultá-la como um oráculo.
