...
I'm a full-stack developer, not a security engineer. I know the basics (hash passwords, use HTTPS, keep secrets out of the frontend), but I also know there's a gap between "I think it's fine" and "I've actually tested it." Security audits from professional firms cost thousands and take weeks. I didn't have that budget or timeline.
...
I wasn't trying to replace a professional penetration test. I wanted a thorough first pass, the kind of review that catches obvious and not-so-obvious mistakes before a human specialist would even start.

What makes Claude Code interesting for this is that it can read your entire codebase, understand the architecture, follow data flows across layers, and think like an attacker. It goes past pattern matching for `eval()` or hardcoded secrets into actual business logic: questions like "what happens if a user sends this ID that belongs to someone else?" or "what if this integer field has no upper bound?"
...
The audit is only as good as the brief. I didn't just say "find security bugs." I wrote a detailed prompt that frames Claude Code as a specific role, gives it a structured analysis framework, and tells it exactly what output I expect.

Here's the full prompt (long on purpose):
...
I also gave it the test credentials and the live API endpoint, so it could make actual HTTP requests and confirm findings against the running application instead of guessing from source code alone.

The framing carried most of the weight: **"You are not a developer checking boxes — you are a security specialist who thinks like an attacker."** That question produces a completely different analysis than "does this code follow best practices?", and a far more useful one.

The structure paid off directly. Every specific instruction in the brief came back as a specific, actionable section in the report; the few things I left vague came back vague.
...
Claude Code traced the data flow through the routine check endpoint and found that while the request included a `userId` parameter, the service **never actually used it** to verify ownership. Any authenticated user could check habits in any other user's routine, and the XP would be awarded to the attacker rather than the routine owner.

This was a classic IDOR (Insecure Direct Object Reference), but a subtle one, because the `userId` was in the method signature: it looked like it was being used. You had to read the implementation carefully to notice it was silently ignored.
...
**Most vulnerabilities are logic bugs.** Nobody was going to SQL-inject Beyou; JPA parameterizes everything. The real risks lived in the business logic around XP, ownership, and input bounds.
...
**AI-assisted auditing accelerates, and the discussion is half the value.** Claude Code found things I wouldn't have thought to check, and it found them fast. The phase where I explained my architecture and we refined the findings together mattered just as much. The tool gives its best when you argue with it like a colleague instead of consulting it like an oracle.
