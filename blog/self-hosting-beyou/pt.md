...
Duas coisas me surpreenderam no caminho. A primeira é que a coisa mais valiosa da máquina nunca foi o banco — era o arquivo `.env` de 5 KB, o único lugar onde o segredo do OAuth, a senha do e-mail e todas as chaves de API existiam. Perder o banco custa dados; perder esse arquivo significa reemitir cada credencial que eu tenho enquanto o app está fora do ar. A segunda é que a minha primeira versão silenciosamente nunca apagava nada: o restic agrupa snapshots por caminho quando aplica a política de retenção e, como eu montava cada execução num diretório temporário novo, cada snapshot caía num grupo só dele e o "manter 7 diários" mantinha todos para sempre. Só peguei isso porque rodei a coisa três vezes seguidas e contei.
