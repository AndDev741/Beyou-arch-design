...
Then the chain got shorter for a reason that had nothing to do with quality. Publishing on Google Play meant writing a privacy policy, and writing a privacy policy meant naming every company that receives a user's message. Two of these are established in China, which has no adequacy decision from the European Commission — and I am a controller established in Portugal, so those requests are transfers with no lawful route. Z.ai and DeepSeek came out. Production runs Mistral and Gemini, which means Gemini is now the link that never skips.

Losing the paid safety net cost less than I expected, because the two remaining tiers were already absorbing everything. What it did cost is the comfort of a last resort, so the config has a second list beside the order: a blocklist those two names sit on. The order is an environment variable anybody can widen, and I did not want a promise printed in a privacy policy to depend on me remembering why the order was narrow. GLM and DeepSeek still run in development, where every habit and goal is invented.
