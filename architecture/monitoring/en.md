...
Everything in it is created and reconciled by a bootstrap script that treats itself, not the UI, as the source of truth: the organization, the team, the four projects, twelve uptime monitors probing container addresses from inside the network (the backend's health endpoint — a named group that includes only local checks, never the full endpoint, so no indicator added later can put a network call in front of the probe, the web frontend's port, both databases, Valkey, Loki, Alloy, Prometheus, Grafana, optionally Watchtower, and GlitchTip's own health for the status page), three heartbeat monitors, and four alert rules. Exactly one rule per project, because the uptime notification path joins through the project and a second rule would mail every event twice.

The heartbeats are the subtle ones. The snapshot scheduler checks in only after a completed cycle, and GlitchTip alerts when 90 minutes pass without a check-in. The check is inverted on purpose: a wedged scheduler keeps the health endpoint green while silently writing no snapshots, so the alert fires on the absence of success rather than the presence of failure.

The engagement-nudge pass has the same heartbeat, and needs it more. A snapshot job that stops eventually shows up as missing history; a nudge job that stops looks exactly like a quiet week, because nobody complains about mail they did not receive. Its monitor is opt-in in the bootstrap script and deliberately so: the pass returns before checking in while `engagement.enabled` is false, so a monitor created ahead of that flag would page every 90 minutes about a job that is off on purpose. `GLITCHTIP_NUDGE_HEARTBEAT=1` belongs in the same change that turns the sender on.

The other two watch the backups, and they are inverted for the same reason taken further: the nightly job and the weekly restore drill both check in only on success, so a backup that stops running raises no error, fails no request, and turns nothing red. There are two rather than one because "uploaded without error" and "actually restores" are separate claims, and a backup that satisfies only the first is the ordinary way this fails.
