...
Two web pages, habits and tasks, keep the list they render in local state instead of reading the slice. That is older than the shared store and not a problem in itself. What was a problem is that their fetches, and the refetch their forms run after a save, wrote only to that local list: the persisted slice kept the old name, and every routine row on the dashboard and on the focus screen resolves name, icon and phrase from these slices by id. So a habit renamed on its own page kept its old name in focus mode until that screen's own fetch happened to land. Both pages now fetch through one hook per entity (`useSyncedHabits`, `useSyncedTasks`) that hands the same list to the store, and a third caller cannot skip the dispatch because the api module is no longer the door. The dashboard's loader learned the matching rule on the read side: a fetch that failed is not applied, where it used to dispatch `undefined` into the slice and empty every name on screen after one throttled request. The focus screen also joins the auto-refresh policy the other data screens follow (coming back to the tab or the app, the day turning over, time passing), because it is the screen left open longest and a rename made on another device never reached it.
