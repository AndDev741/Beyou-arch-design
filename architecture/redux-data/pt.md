...
Duas páginas do web, hábitos e tarefas, guardam a lista que renderizam em estado local em vez de ler o slice. Isso é mais antigo que a store compartilhada e não é um problema por si. O problema era que as buscas delas, e a rebusca que os formulários fazem depois de salvar, escreviam só nessa lista local: o slice persistido ficava com o nome antigo, e toda linha de rotina no dashboard e na tela de foco resolve nome, ícone e frase a partir desses slices pelo id. Então um hábito renomeado na própria página continuava com o nome antigo no modo foco até a busca daquela tela acontecer de chegar. As duas páginas agora buscam por um hook por entidade (`useSyncedHabits`, `useSyncedTasks`) que entrega a mesma lista à store, e um terceiro chamador não consegue pular o dispatch porque o módulo de api deixou de ser a porta. O loader do dashboard aprendeu a regra correspondente do lado da leitura: uma busca que falhou não é aplicada, onde antes despachava `undefined` no slice e apagava todos os nomes da tela depois de um pedido barrado por limite. A tela de foco também entra na política de auto-refresh que as outras telas de dados seguem (voltar à aba ou ao app, o dia virar, o tempo passar), porque é a tela que fica aberta por mais tempo e um rename feito noutro aparelho nunca chegava a ela.
