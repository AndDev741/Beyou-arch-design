...
Cinquenta ferramentas agrupadas por domínio: CRUD completo de hábitos, categorias, tarefas e metas (mais completar, aumentar e diminuir meta, e uma ferramenta de mover-para-debaixo que muda o pai de uma meta na árvore pelo nome, sem reenviar todos os campos), construção de rotina (criar, edições dirigidas, edição de substituição total, adicionar e remover item), rotinas em lista (criar e substituir por inteiro, as duas recebendo um array plano de itens e nenhum horário), schedules, a rotina de hoje com check e skip, as micro-tarefas do Modo Foco (listar, adicionar, marcar, fixar, apagar, reordenar) e a visão do dia, leitura e ajuste da configuração do usuário, humor diário (um escritor e um leitor), dois escritores de memória e envio de feedback.

Três ausências nessa lista são deliberadas. **Um ciclo de timer não pode ser escrito.** Um ciclo é o registro de que alguém de fato passou por ele, e o cliente só reporta ciclo que chegou ao fim, então uma ferramenta capaz de gravá-los deixaria o agente inventar história que a pessoa nunca viveu — o mesmo raciocínio que mantém o check-in atrás de um pedido explícito. Ciclos são legíveis pela visão do dia e nada além. E **nenhuma ferramenta adivinha a qual entrada de rotina uma micro-tarefa pertence**: um id de entrada ausente é recusado em vez de assumido, porque uma linha escrita na entrada errada é silenciosa e a pessoa só descobre depois, sem nada que explique. A recusa nomeia a entrada aberta no Modo Foco, quando existe uma, que costuma ser a que o modelo quis dizer. E **o diário em si não é legível**: a ferramenta de histórico devolve a data de cada dia, o nível e se algo foi escrito, nunca as palavras. Quem escreve longamente para si não pediu que um modelo lesse, e uma ferramenta capaz disso faria de cada registo parte do próximo prompt. Se quiserem que o assistente veja um dia, colam o texto.
...
- **Todo argumento de escrita é revalidado** com as mesmas constraints Jakarta dos DTOs REST, e violações voltam listando cada campo reprovado para o modelo se corrigir. Argumentos enviados sem o objeto que os embrulha vinculam o DTO inteiro como null, o que antes aparecia como uma reclamação ilegível do próprio validador; agora esse caso diz qual formato o tool esperava. Horários também são lidos com tolerância: um modelo que escreve 7:00 em vez de 07:00 é entendido, e algo realmente impossível de ler nomeia o campo e o formato em vez de estourar. O que justifica o cuidado é o custo de errar — a cadeia de tier gratuito tem cota diária e tools nunca re-executam no meio do turno, então cada autocorreção é uma chamada inteira gasta em erro de binding.
